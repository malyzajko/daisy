...
import Real._
@strictfp
object instantaneousCurrent_order {


  def instantaneousCurrent_order(t: Real, resistance: Real, frequency: Real, inductance: Real, maxVoltage: Real): Real = {
    ...
    val _ret4: Real = (maxCurrent * cos(((((2.0 * pi) * frequency) * t) + theta)))
    _ret4
  } ensuring((res) => (res +/- 0.0028611616721834106))
