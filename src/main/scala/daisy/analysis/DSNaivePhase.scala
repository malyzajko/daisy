  ...
  def computeResults(expr: Expr,
    ...
    (Map[Expr, DSAbstraction], Map[Expr, DSAbstraction]) = {
      ...
      case x@VectorElement(v, index) =>
        val (indexRanges, indexErrs) = eval(index, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        ...
        val (vRanges, vErrors) = eval(v, indexRanges, indexErrs, updIndices, forceEval, unrollingScope)
      ...
      case x@MatrixElement(m, irow, icol) =>
        val (iRowRanges, iRowErrs) = eval(irow, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val (iColRanges, iColErrs) = eval(icol, iRowRanges, iRowErrs, indices, forceEval, unrollingScope)
        ...
        val (mRanges, mErrors) = eval(m, iColRanges, iColErrs, updIndices, forceEval, unrollingScope)
      ...
      case x@RowOfMatrix(m, row) =>
        ...
        val (indexRanges, indexErrs) = eval(row, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        ...
        val (mRanges, mErrors) = eval(m, indexRanges, indexErrs, updIndices, forceEval, unrollingScope)
      ...
      case x@SubVector(v, from, to) =>
        val (fromRanges, fromErrs) = eval(from, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val (toRanges, toErrs) = eval(to, fromRanges, fromErrs, indices, forceEval, unrollingScope)
        ...
        val (vRanges, vErrors) = eval(v, toRanges, toErrs, indsToTake, forceEval, unrollingScope) // in case v is an expression
      ...
      case x@PadVector(v, Int32Literal(padSize)) =>
        val (vRanges, vErrors) = eval(v, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        // change indices
      ...
      case x@PadMatrix(m, Int32Literal(padRows), Int32Literal(padCols)) =>
        val (mRanges, mErrors) = eval(m, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        // change indices
      ...
      case x@ZipVectors(lhs, rhs) =>
        ...
        val (lhsRanges, lhsErrors) = eval(lhs, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val (rhsRanges, rhsErrors) = eval(rhs, lhsRanges, lhsErrors, indices, forceEval, unrollingScope)
      ...
      case x@AppendElement(v, el) if isVector(v) =>
        ...
        val (eltRanges, eltErrors) = eval(el, vRanges, vErrors, indices, forceEval, unrollingScope)
      ...
      case x@AppendElement(m, el) if isMatrix(m) =>
        ...
        val (eltRanges, eltErrors) = eval(el, mRanges, mErrors, indices, forceEval, unrollingScope)
      ...
      case x@Cast(n, Int32Type) => // for n.intValue()
        ...
        val (nRanges, nErrors) = eval(n, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
      ...
      case x@VectorFromList(list, length) =>
        val (listRanges, listErrs) = (0 until length).foldLeft((rangesAbstr, errorsAbstr))({
          case (acc, i) =>
            val listElt = list(i)
            val (eltRanges, eltErrors) = eval(listElt, acc._1, acc._2, indices + (x -> Set(VectorIndex(i))), forceEval, unrollingScope)
          ...
          case (acc, i) =>
            (0 until numCols).foldLeft(acc)({
              case (accC, j) =>
                ...
                val (eltRanges, eltErrors) = eval(matrixElt, accC._1, accC._2, indices + (x -> Set(MatrixIndex(i, j))), forceEval, unrollingScope)
      ...
      case x@EveryNthVector(v, Int32Literal(n), Int32Literal(from)) =>
        val (vRanges, vErrs) = eval(v, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val origDSRange = vRanges(v)
      ...
      case x@EveryNthMatrix(m, Int32Literal(n), Int32Literal(from)) =>
        val (mRanges, mErrs) = eval(m, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val origDSRange = mRanges(m)
      ...
      case x@SizeLength(ds) =>
        val (dsRanges, dsErrors) =
          if (rangesAbstr.contains(ds))
            ...
            eval(ds, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
      ...
      case x@IfExpr(cond, thenn, elze) =>
        ...
        val (condRanges, condErrs) = eval(norm, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val condIntervals = Seq(condRanges(norm).fullInterval) // todo something better?
        val holds = conditionHolds(cond, condIntervals)
        if (holds) {
          val (thenRanges, thenErrs) = eval(thenn, condRanges.removed(norm), condErrs.removed(norm), indices, forceEval, unrollingScope)
          (thenRanges + (x->thenRanges(thenn)), thenErrs + (x->thenErrs(thenn)))
        } else {
          val (elseRanges, elseErrs) = eval(elze, condRanges.removed(norm), condErrs.removed(norm), indices, forceEval, unrollingScope)
      ...
      case x@FunctionInvocation(fdId, params, args, returnType) if fdId == currentFncId =>
        val (argsRanges, argsErrs) = args.foldLeft((rangesAbstr,errorsAbstr))({
          case (acc, argExpr) =>
            ...
            eval(argExpr, acc._1, acc._2, indices, forceEval, unrollingScope)
        })
        ...
        // todo indices?
        val (resR, resE) = eval(expr, rangesAbstr++updateRange.toMap, errorsAbstr++updateErrs.toMap, indices, forceEval, true ) // todo inner scope?
            ...
            val (updRanges, updErrors) = dsRanges(ds).indices.foldLeft((dsRanges,dsErrors))({
              case (acc, ind) =>
                eval(cond, acc._1 + (lambda -> dsRanges(ds)), acc._2 + (lambda -> dsErrors(ds)),
                  indices ++ Map(lambda -> Set(ind), cond -> Set(ind), ds -> Set(ind)), forceEval, unrollingScope)
            })
        ...
        val (bodyRanges, bodyErrs) = dsRanges(ds).indices.map({
          ind =>
            val (rng, err) = eval(body, dsRanges + (lambda -> dsRanges(ds)), dsErrors + (lambda -> dsErrors(ds)),
              indices ++ Map(lambda -> Set(ind), body -> Set(ind)), forceEval, unrollingScope)
      ...
      case x@Sum(ds, init) if !forceEval && isVector(ds) =>
        val (dsRange, dsErrs) = eval(ds, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val (initRange, initErrs) = eval(init, dsRange, dsErrs, indices, forceEval, unrollingScope)
        ...
        val chunkedUnrollRes = unrollChunks.foldLeft((initRange, initErrs, init))({
          case (acc, inds) =>
            ...
            val (foldRanges, foldErrs) = eval(unrolledFold, acc._1, acc._2, indices, forceEval, unrollingScope)
      ...
      case x@FoldIter(ds, init, Lambda(args, body)) if !forceEval && isVector(ds) =>
        val (initRange, initErrs) = eval(init, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        ...
        val chunkedUnrollRes = unrollChunks.foldLeft((initRange, initErrs, init))({
          case (acc, inds) =>
            ...
            val (foldRanges, foldErrs) = eval(unrolledFold, acc._1, acc._2, indices, forceEval, true)
      ...
      case x@FoldIter(ds, init, Lambda(args, body)) if !forceEval && isMatrix(ds) =>
        ...
        val chunkedUnrollRes = unrollChunks.foldLeft((initRange, initErrs, init, indices))({ case (acc, rowInds) =>
          ...
          val (foldRanges, foldErrs) = eval(unrolledFold, acc._1, acc._2, acc._4, forceEval, true)
      ...
      case x@SlideReduceIter(ds, Int32Literal(size),Int32Literal(step), Lambda(args, body)) if !forceEval && isVector(ds) =>
        ...
        val (xRangeMap, xErrMap) = slideResultIndices.map({
          case index =>
            ...
            val (iterRanges, iterErrs) = eval(body, updRangesDS, updErrsDS, indices + (iterV -> iterDSIndices.toSet), forceEval, unrollingScope)
      ...
      case x@SlideReduceIter(ds, Int32Literal(size),Int32Literal(step), Lambda(args, body)) if !forceEval && isMatrix(ds) =>
        ...
        val (xRangeMap, xErrMap) = slideResultIndices.map({
          case index@MatrixIndex(i,j) =>
            ...
            val (iterRanges, iterErrs) = eval(body, updRangesDS, updErrsDS, indices + (iterM -> iterDSIndices.toSet), forceEval, unrollingScope)
        ...
        val (xRangeMap, xErrMap) = (0 until dsRangeAbs.numRows).map(i =>
          {
            ...
            val (iterRanges, iterErrs) = eval(body, updRangesDS, updErrsDS, indices + (iterV -> vectorIndices), forceEval, unrollingScope)
      ...
      case x@EnumSlideFlatMap(ds, Int32Literal(size), Lambda(args, body)) if !forceEval && isVector(ds) =>
        ...
        val (xRangeMap, xErrMap) = (0 until steps).map(i =>
        {
          ...
          val (iterRanges, iterErrs) = eval(body, updRangesDS, updErrsDS, indices + (iterV -> sliceIndices.toSet), forceEval, unrollingScope)
      ...
      case x@Let(id, value, body) =>
        val (valRanges, valErrs) = eval(value, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope)
        val binder =
          if (isVector(value))
            VectorLiteral(id)
        ...
        val (letRanges, letErrs) = eval(body, valRanges + (binder -> valRanges(value)), valErrs + (binder -> valErrs(value)), indices + (binder -> valRanges(value).indices), forceEval, unrollingScope)
      ...
      case x@CrossProduct(lhs, rhs) if isVector(lhs) && isVector(rhs) =>
        ...
        val (fstRanges, fstErrs) = eval(firstElt, rhsRanges, rhsErrors, indices, forceEval, unrollingScope)
        val secondElt = Plus(Times(a,d), Times(b,c))
        val (sndRanges, sndErrs) = eval(secondElt, rhsRanges, rhsErrors, indices, forceEval, unrollingScope)
        ...
        val rows: (Seq[(Set[Index], MPFRInterval)], Seq[(Set[Index], MPFRInterval)]) = (0 until rhsRanges(lhs).numRows).map(i => {
          ...
          val sumExpr = toSum.tail.foldLeft((toSum.head._1.asInstanceOf[Expr], rhsRanges ++ toSum.head._2, rhsErrors ++ toSum.head._3))({
            case (acc, x) =>
              ...
              val tmp = eval(sumE, acc._2 ++ x._2, acc._3 ++ x._3, indices, forceEval, unrollingScope)
              (sumE, tmp._1, tmp._2)
          })
      ...
      case ArithOperator(es, fnc) if !forceEval && es.length == 1 && hasDsType(es.head) =>
        val arg = es.head
        val (argRanges, argErrors) = eval(arg, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope) // todo indices: for input DS is already there, others?
        val operatorApplied = fnc(es)
        val (opRanges, opErrs) = argRanges(arg).indices.map({ indexInArg =>
          val (partRanges, partErrs) = eval(operatorApplied, argRanges, argErrors, indices + (arg -> Set(indexInArg)), true, unrollingScope)
      ...
      case ArithOperator(es, fnc) if !forceEval && es.length == 1 && isDsExpr(es.head) && es.head.getType == RealType =>
        ...
        val (argRanges, argErrors) = eval(arg, rangesAbstr, errorsAbstr, indices, forceEval, unrollingScope) // todo indices: for input DS is already there, others?
        val operatorApplied = fnc(es)
          eval(operatorApplied, argRanges, argErrors, indices, true, unrollingScope) // todo indices ? + (arg -> Set())
      ...
      case ArithOperator(es, fnc) if !forceEval && es.forall(hasDsType) =>
        ...
        val (lhsRanges, lhsErrors) = eval(lhs, rangesAbstr, errorsAbstr, lhsIndices, forceEval, unrollingScope)
        val (rhsRanges, rhsErrors) = eval(rhs, lhsRanges, lhsErrors, rhsIndices, forceEval, unrollingScope) // todo is it ok to call with results of lhs eval?
        ...
        val overlap = lhsRanges(lhs).indices.map(lIndex => {
          ...
          val newRanges = eval(operatorApplied, rhsRanges, rhsErrors, indices ++ indConfig, true, unrollingScope)
      ...
      case ArithOperator(es, fnc) if !forceEval && hasDsType(es.head) && (es(1).getType == RealType || es(1).getType == Int32Type) =>
        ...
        val lhsIndices = if (rangesAbstr.contains(lhs)) indices + (lhs -> rangesAbstr(lhs).indices) else indices
        val (lhsRanges, lhsErrors) = eval(lhs, rangesAbstr, errorsAbstr, lhsIndices, forceEval, unrollingScope)
        val (rhsRanges, rhsErrors) = eval(rhs, lhsRanges, lhsErrors, lhsIndices, forceEval, unrollingScope) // todo is it ok to call with results of lhs eval?

        val operatorApplied = fnc(es)
        val (opRanges, opErrs) = rhsRanges(lhs).indices.map({ indexLhs =>
            val (partRanges, partErrs) = eval(operatorApplied, rhsRanges, rhsErrors, indices + (lhs -> Set(indexLhs)), true, unrollingScope)
      ...
      case ArithOperator(es, fnc) if !forceEval && es.exists(isDsExpr) =>
        ...
        eval(operatorApplied, rhsRanges, rhsErrors, indConfig, true, unrollingScope)
      ...
      case x if !isDsExpr(x) || forceEval =>
        ...
        val updatedRangeAbstr = updateDSWithRanges(x, indices, rangesAbstr, intermedRanges, emptyPath)
        val updatedErrorAbstr = updateDSWithRanges(x, indices, errorsAbstr, intermedErrs, emptyPath)
    ...
    val (rangeNew, errNew) = eval(expr, dsas ++ realRanges, dsErrors ++ realErrors, dsIndices ++ realIndicesDummy, false, false)
