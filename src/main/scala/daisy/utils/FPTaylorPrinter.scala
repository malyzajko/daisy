...
import daisy.lang.Types.{FinitePrecisionType, TypeTree}
  ...
  override def pp(tree: Tree, parent: Option[Tree])(implicit lvl: Int): Unit = {
    ...
    tree match {
      ...
      case fd: FunDef =>
        ...
        fd.params.foreach {
          vd => {
            ...
            val range = ctx.specInputRanges(fd.id)(vd.id)
              .toBigString
            //            val error = ctx.specInputErrors(fd.id)(vd.id)
            varDefs = varDefs + "\t real " + {
              vd.id.uniqueNameDelimited('_'.toString)
            } + " in " + range + /*"+/- " + error + */ ",\n"
          }
        }

        sb.append(varDefs.substring(0, varDefs.length - 2) + "\n;\n\n")
        ...
        sb.append(s"Expressions \n\n ${returnId.uniqueNameDelimited('_'.toString)}; \n\n")
