...
class InlinedDaisyPrinter(buffer: Appendable, ctx: Context, val generateDaisyInput: Boolean = true,
  val importString: String = "import daisy.lang._\nimport Real._") extends CodePrinter(buffer) {
  ...
  override def pp(tree: Tree, parent: Option[Tree])(implicit lvl: Int): Unit = {
    ...
    tree match {
      ...
      case FinitePrecisionType(Float64) => sb.append("Real")
