...
case class AffineForm(x0: Rational, noise: Seq[Deviation]) extends RangeArithmetic[AffineForm] {
  ...
  override def toString = "[%s,%s]".format(toInterval.xlo.toDouble, toInterval.xhi.toDouble)
  def toSmallString = "[%.3f,%.3f]".format(toInterval.xlo.toDouble, toInterval.xhi.toDouble)
  def toBigString = "[%.18f,%.18f]".format(toInterval.xlo.toDouble, toInterval.xhi.toDouble)

  def longString: String =
    "%s +/- %s".format(x0.toString, noise.mkString(", "))

  def detailString = x0.toDouble + " +/- " + radius.toDouble
