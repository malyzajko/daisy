...
/**
  ...
  Prerequisites:
    ...
    - DataflowPhase
 */
object ApproxPhase extends DaisyPhase with CASTExtractor with opt.CostFunctions {
  ...
  override def runPhase(ctx: Context, prg: Program): (Context, Program) = {
    ...
    val metalibmFolder = new File(metalibmPath)
    if (metalibmFolder.isDirectory) { // always a dir
      metalibmFolder.listFiles.filter(_.getName.contains("_id_sollya_git_devel_")).foreach(_.delete())
    }
      ...
      if (updates.contains(fnc._1)) {
        ...
        fnc._1 -> updList.foldLeft(fnc._2)((accmap, upd) => accmap.map({ x =>
          ...
          def replace(e: Expr, p: PathCond): Expr = {
            (e, p) match {
            case (node@ElemFnc(_, _), path) if mapApprox.contains(node, path) && mapApprox(node, path) == upd.newNode => upd.newNode // plug in Approx node
            ...
            case (node@Cast(x@ElemFnc(_, _),_), path) if mapApprox.contains(x, path) && mapApprox(x, path) == upd.newNode => upd.newNode
  ...
  def findReplace(fnc: FunDef, context: Context): (FunDef, Map[Identifier, Seq[ContextUpdate]], Map[(Expr, PathCond), ApproxPoly]) = {
    ...
    def find(e: Expr, p: PathCond, ctx: Context, mapDuplicatesApprox: Map[(String, Interval, TypeTree), ApproxPoly], mapApprox: Map[(Expr, PathCond), ApproxPoly], ctxUpdates: Map[Identifier, Seq[ContextUpdate]]): (Expr, Map[Identifier, Seq[ContextUpdate]], Map[(String, Interval, TypeTree), ApproxPoly], Map[(Expr, PathCond), ApproxPoly]) = (e, p) match {
      ...
      case x @ (castOfElem @ Cast(elemFnc @ ElemFnc(arg, _), tpe), path) =>
      ...
      case x @ (elemFnc @ ElemFnc(arg, _), path) =>
        ctx.reporter.result(s"approximating fnc: ${fnc.id} subexpression $elemFnc")
      ...
      case x @ (ArithOperator(es_old, recons), path) =>
        ...
        val (es, ctxNewUpdates, updDuplMapApprox, updMapsApprox) = es_old.drop(1).foldLeft(init)((acc, arg) => {
          ...
          (acc._1 :+ tmp._1, ctxUpdMap.mapValues(_.distinct).toMap, acc._3 ++ tmp._3, acc._4 ++ tmp._4)
        })
  ...
  def callMetalibm(settings: MetalibmSettings, expectedReturnType: TypeTree): (Option[FunDef], Rational) = {
    ...
    val problemdef = new PrintWriter(new File(s"${metalibmPath}/problemdefApprox.sollya"))
    problemdef.write(settings.toProblemDef)
    problemdef.close()

    /* Run the problemdef, return results or return None if a time out occurs*/
    var lastline: String = ""

    val f: Future[Unit] = Future {
      val problemDefRes = Runtime.getRuntime().exec("./metalibm.sollya problemdefApprox.sollya",
        null, new File(metalibmPath))  //run in metalibm-approx directory
    ...
    try {
      ...
      case e: java.util.concurrent.TimeoutException =>
        ...
        throw new DaisyFatalError(Some("Metalibm timed out"))
        return null
      case e: java.io.IOException =>
        reporter.warning(e.getMessage())
        throw new DaisyFatalError(Some("There was a problem calling Metalibm."))
        return null
      case e: Exception =>
    ...
    if (output.length > 2 && output(1) == "true") {
      ...
      // println(implementationFile)
      // println(s"error returned: ${Rational.fromString(implErr)}")
      (CodeParser.getFunctionDef(implementationFile, settings, expectedReturnType), Rational.fromString(implErr))
    } else {
      reporter.warning("Metalibm couldn't compute the approximation, last line: " + lastline)
      throw new DaisyFatalError(Some("Metalibm couldn't compute the approximation, last line: " + lastline))
    }
  }
