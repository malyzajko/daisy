...
import opt.MetalibmSettings

import java.nio.charset.CodingErrorAction
import scala.collection.immutable.Seq
import scala.io.{Codec, Source}

trait CASTExtractor extends CCodeExtraction with CReqExtraction {

  object SpecParser extends ReqParser {
    /**
      * Parse specification from the problemdef.sollya file
      * @param specFile name of the file with specification (e.g. testcases/parse/testspec.txt)
      * @return
      */
  ...
  object CodeParser extends ExpressionParser {
    def parse(progFile: String): Option[Seq[Tree]] = {
      val codec: Codec = Codec("UTF-8")
      codec.onMalformedInput(CodingErrorAction.REPLACE)
      codec.onUnmappableCharacter(CodingErrorAction.REPLACE)
      val fileContents = Source.fromFile(progFile)(codec).getLines.mkString // e.g. "testcases/parse/testloop.c"
