  ...
  class Extraction(units: List[CompilationUnit]) {
    ...
    private def extractTree(tr: Tree)(implicit dctx: DefContext): DaisyExpr = {
      ...
      val res = current match {
        ...
        case ExImplicitInt2RealOp(tree) =>
          extractTree(tree)

        case ExImplicitDouble2RealOp(tree) =>
          extractTree(tree)
        ...
        case ExPlusMinus(l, r)    =>
          AbsError(extractTree(l), extractTree(r))
        case ExPow(l, r)      =>
        ...
        case ExCall(tr, sym, args) if (args.length == 1) =>
          ...
          (lhs, sym.name.decoded, rhs) match {
            case (IsTyped(a1, a1Type), "+", IsTyped(a2, a2Type))
              if ((a1Type == RealType || a1Type == Int32Type) && (a2Type == RealType || a2Type == Int32Type)) =>
              Plus(a1, a2)
            case (IsTyped(a1, a1Type), "-", IsTyped(a2, a2Type))
              if ((a1Type == RealType || a1Type == Int32Type) && (a2Type == RealType || a2Type == Int32Type)) =>
              Minus(a1, a2)
