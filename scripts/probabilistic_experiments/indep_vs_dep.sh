...
# Runs the probabilistic analysis with 4 DSI and 8000 outer subdivision for float32 independent inputs
echo "******************** Probabilisitic Analysis for uniform independent inputs ********************"
for file in "${files[@]}"
do
  echo ${file}
...
# Runs the probabilistic analysis with 4 DSI and 8000 outer subdivision for float32 dependent inputs
echo "******************** Probabilisitic Analysis for uniform dependent inputs ********************"
for file in "${files[@]}"
do
  echo ${file}
  time ./daisy --probabilistic $file --thresholdFile=${thres} --precision=Float32 --dependent
done