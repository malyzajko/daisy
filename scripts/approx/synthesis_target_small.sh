...
# and then compile them usign Xilinx Vivado HLS.
#!/bin/bash --posix
# This script will run Daisy (and Metalibm) to synthesize approximate programs
# for benchmarks with small target errors, and then compile them usign Xilinx Vivado HLS.
#
# Run the script form the current directory scripts/approx

base="output"
file="testcases/approx/Benchmarks_small.scala"

cd ../..
approx_folder="${base}/source_small"
mkdir $approx_folder
xilinx_folder="${base}/results_small"
mkdir $xilinx_folder

logdir="${base}/logs"
mkdir $logdir
if [ -e "${logdir}/daisy_small.log" ]; then
  rm -rf  "${logdir}/daisy_small.log"
fi
...
do
  echo "******* running Daisy on ${fnc}" | tee -a -i "${logdir}/daisy_small.log"
  ...
  (time $script --silent --rangeMethod=interval --errorMethod=affine\
    ...
    --functions="${fnc}" ${file}) 2>&1 | tee -a -i "${logdir}/daisy_small.log"
...
declare -a functionsMixedUni=("predictGaussianNB" "predictMLPLogistic")
# predictGaussianNB: mixed precision for global error budget, uniform for polynomials
for fnc in "${functionsMixedUni[@]}"
do
	echo "******* running Daisy on ${fnc}" | tee -a -i "${logdir}/daisy_small.log"
	(time $script --silent --rangeMethod=interval --errorMethod=affine\
...
# predictSVC mixed precision
fnc="predictSVC"
echo "******* running Daisy on ${fnc}" | tee -a -i "${logdir}/daisy_small.log"
(time $script --silent --rangeMethod=interval --errorMethod=affine\
    --approx --apfixed --lang=C --precision=Fixed64\
    --cost=combined --mixed-tuning --choosePrecision=fixed \
    --functions="${fnc}" ${file}) 2>&1 | tee -a -i "${logdir}/daisy_small.log"
...
if [[ $OSTYPE == "darwin"* ]]; then
    ...
    printf "======== Small target errors ============\n"
    python3 scripts/approx/ParseLogs.py "small" "macos"
    exit 0
fi

declare -a functions=( "${functionsUni[@]}" "${functionsMixedUni[@]}" "predictSVC" )
...
# Parse Daisy log files and reports for a readable output
printf "\n===================== Small target errors ==========================\n"
python3 scripts/approx/ParseLogs.py "small"
